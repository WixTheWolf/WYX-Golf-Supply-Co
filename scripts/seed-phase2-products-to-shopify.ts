/**
 * seed-phase2-products-to-shopify.ts
 *
 * Seeds Phase 2 products to Shopify as ACTIVE listings.
 * Reuses the same GraphQL mutations as directSeed.ts.
 *
 * Usage:
 *   npm run seed:phase2-products
 */

import { phase2Products } from '../lib/shopify/phase2Products';
...
async function seedOne(product: DirectProduct, publish: boolean): Promise<SeedResult> {
  ...
  // 3. Set price on auto-created variant
...
async function main() {
  ...
  console.log(`\n🏌️  WYX Golf — Phase 2 Product Seed`);
  console.log(publish ? '🟢 Creating as ACTIVE' : '🟡 Creating as DRAFT');
  console.log(`📦 ${phase2Products.length} products\n`);

  const results: SeedResult[] = [];

  for (const product of phase2Products) {
