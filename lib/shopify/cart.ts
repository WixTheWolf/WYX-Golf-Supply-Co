...
export async function createCartWithLines(lines:{merchandiseId:string;quantity:number}[]){if(!hasShopify)throw new Error('Real checkout requires Shopify env vars');const d=await shopifyFetch<any>(CART_CREATE,{lines});errs(d);return cart(d.cartCreate.cart)}
export async function addLines(cartId:string,lines:{merchandiseId:string;quantity:number}[]){const d=await shopifyFetch<any>(CART_LINES_ADD,{cartId,lines});errs(d);return cart(d.cartLinesAdd.cart)}
