    ...
    <>
      <section className="collection-hero">
        <div>
          ...
          <div className="actions"><Link className="button primary" href="#popular-picks">Shop Popular Picks</Link><Link className="button secondary dark" href="/deals">See The First-Order Offer</Link></div>
        </div>
        <div className="collection-proof"><span>Useful golf gear</span><span>Secure checkout</span><span>First-order offer</span><span>Built for everyday rounds</span></div>
      </section>

      <section className="deal-strip" aria-label="Popular golf product focus"><span>Gifts under $60</span><span>Small bag upgrades</span><span>Useful accessories</span><span>Easy first cart</span></section>

      <section id="popular-picks" className="section product-section">
        <div className="section-heading split"><div><p className="eyebrow">Best Current Picks</p><h2>Ready To Buy Now.</h2></div><Link className="text-link" href="/products">Browse Full Catalog</Link></div>
      ...
      <section className="section">
        <div className="section-heading split"><div><p className="eyebrow">Better Bag Basics</p><h2>Why Golfers Keep These Around.</h2></div><Link className="text-link" href="/golf-gifts-under-60">{underSixty.length} Under-$60 Picks</Link></div>
        <div className="intent-grid">{scoutingBoard.map(([title, description]) => <article className="marketing-card" key={title}><span>{title}</span><p>{description}</p></article>)}</div>
      </section>

      <EmailCapture source="popular-products-2026" campaign="popular_golf_products_2026" title="WANT THE BEST NEW WYX PICKS?" body="Join the list for fresh golf gear, premium finds, and the products worth knowing about." />

      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
        '@context': 'https://schema.org', '@type': 'CollectionPage', name: 'Popular Golf Products 2026', description: metadata.description,
        url: `${siteUrl}/popular-golf-products-2026`,
        mainEntity: topProducts.map((product) => ({ '@type': 'Product', name: product.title, url: `${siteUrl}/products/${product.handle}`, image: product.featuredImage?.url,
